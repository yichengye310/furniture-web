<template>
  <div class="app-container">
        ...
        <template slot-scope="props">
          <el-table
            ...
            <el-table-column
              ...
              label="操作">
              <template slot-scope="{row}">
                <el-button type="primary" @click="handleUpdateThree(row,props.row)" icon="el-icon-edit" circle></el-button>
                <el-button type="danger" @click="deleteThree(row,props.row)"    icon="el-icon-delete" circle></el-button>
              </template>
            </el-table-column>
          </el-table>
        </template>
    ...
    <el-dialog
      :title="type=='new'?'新增类别':'修改类别'"
      :visible.sync="dialogVisible"
      width="500px">
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="图片" v-if="level==3" required>
          <update-img-for-group :imgType="2" id="tempId" :img-url="form.categoryImg" @add-image="addImage($event)"></update-img-for-group>
        </el-form-item>
...
<script>
  import { categoryLevelTwoListByPage,categoryLevelTwoAdd,categoryLevelThreeAdd,deleteCategoryLevelTwo,deleteCategoryLevelThree, updateCategoryLevelTwo, updateCategoryLevelThree } from '@/api/category'
    ...
    methods: {
      ...
      deleteTwo(row){
        this.$confirm('是否删除类别['+row.categoryName+']?', 'Warning', {
          ...
          deleteCategoryLevelTwo({categoryID:row.categoryID}).then(response => {
            if(response.status==1){
              this.$message({
                type: 'success',
                message: '删除成功！'
      ...
      deleteThree(row,parentRow){
        this.$confirm('是否删除类别['+row.categoryName+']?', 'Warning', {
          ...
          type: 'warning'
            ...
            deleteCategoryLevelThree({categoryID:row.categoryID}).then(response => {
              if(response.status==1){
                this.$message({
                  type: 'success',
                  message: '删除成功！'
