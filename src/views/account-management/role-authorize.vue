<template>
  ...
  <div class="app-container">
    ...
    <el-checkbox-group v-model="checkList">
      <el-row v-for="(temp,index) in tempList">
        <el-col :span="4">
          <el-checkbox :label="temp.name + '(' +index+ ')'" @change="chooseTemp(temp,index)"></el-checkbox>
        </el-col>
        <el-col :span="20">
          <el-row v-for="(tempChild,childIndex) in temp.children" v-if="temp.children.length>0">
            <el-col :span="4">
              <el-checkbox :label="tempChild.name+ '('+index+''+childIndex+')'"
                           @change="chooseTempChild(temp,tempChild,index,childIndex)"></el-checkbox>
            </el-col>
            <el-col :span="20">
              <el-row>
                <el-checkbox :label="tempGrandson.name+ '(' +childIndex+''+grandsonIndex+ ')'"
                             ...
                             v-if="tempChild.children.length>0"></el-checkbox>
              </el-row>
            </el-col>
            <el-divider></el-divider>
          </el-row>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        roleID: getParamer('roleID') != undefined ? Number(getParamer('roleID')) : undefined,
        roleName: getParamer('roleName') != undefined ? decodeURIComponent(getParamer('roleName')) : undefined,
    ...
    created() {
      getRoute({ roleID: this.roleID }).then(response => {
        ...
        if (this.role.permissionContent != null && this.role.permissionContent != '') {
    ...
    mounted() {
      this.roleID = getParamer('roleID') != undefined ? Number(getParamer('roleID')) : undefined
      this.roleName = getParamer('roleName') != undefined ? decodeURIComponent(getParamer('roleName')) : undefined
    },
    methods: {
      //返回上一层
      back() {
      ...
      chooseTemp(temp, index) {
        if (this.permission.indexOf(temp.code) == -1) {
          if (temp.children.length > 0) {
            temp.children.forEach((tempChild, childIndex) => {
              if (this.permission.indexOf(tempChild.code) == -1) {
                ...
                this.checkList.push(tempChild.name + '(' + index + '' + childIndex + ')')
              }
              if (tempChild.children.length > 0) {
                tempChild.children.forEach((tempGrandson, grandsonIndex) => {
                  if (this.permission.indexOf(tempGrandson.code) == -1) {
                    ...
                    this.checkList.push(tempGrandson.name + '(' + childIndex + '' + grandsonIndex + ')')
                  }
                })
              }
            })
          ...
          if (temp.children.length > 0) {
            temp.children.forEach((tempChild, childIndex) => {
              if (this.permission.indexOf(tempChild.code) != -1) {
                this.checkList.splice(this.checkList.indexOf(tempChild.name + '(' + index + '' + childIndex + ')'), 1)
              ...
              if (tempChild.children.length > 0) {
                tempChild.children.forEach((tempGrandson, grandsonIndex) => {
                  if (this.permission.indexOf(tempGrandson.code) != -1) {
                    ...
                    this.checkList.splice(this.checkList.indexOf(tempGrandson.name + '(' + childIndex + '' + grandsonIndex + ')'), 1)
                  }
                })
              }
            })
          ...
          this.checkList.splice(this.checkList.indexOf(temp.name + '(' + index + ')'), 1)
      ...
      chooseTempGrandson(ev, temp, tempChild, index, tempGrandson, childIndex, grandsonIndex) {
