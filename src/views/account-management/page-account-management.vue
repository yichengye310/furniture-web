<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-waves class="filter-item" type="primary"  v-if="permission.indexOf('SearchAccount22343')!=-1" icon="el-icon-search" @click="handleFilter">
        搜索
      </el-button>
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate" v-if="permission.indexOf('AddAccount23573')!=-1">
        新增账号
      </el-button>
    </div>
    ...
    >
      ...
      <el-table-column label="操作" align="center" min-width="240" class-name="small-padding fixed-width" >
        <template slot-scope="{row}">
          <el-button type="primary" size="small" @click="handleUpdate(row)" v-if="permission.indexOf('UpdateAccount2543')!=-1">
            编辑
          </el-button>
          <el-button type="success" size="small" @click="handleUpdatePwd(row)" v-if="permission.indexOf('UpdateAccount2543')!=-1">
            重置密码
          </el-button>
          <el-button  size="small" type="danger" @click="handleModifyStatus(row)" v-if="permission.indexOf('deleteAccount2043')!=-1">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogVisible" title="编辑用户">
      <el-form :model="user" label-width="100px" :rules="rules" ref="ruleForm" label-position="left">
    ...
    <el-dialog :visible.sync="dialogPwdVisible" title="重置用户密码">
      <el-form :model="pwd" label-width="100px"  label-position="left">
        <el-form-item label="账户密码" prop="loginAccount"  >
          <el-input v-model="pwd.password" type="password" style="min-width: 200px;max-width: 800px"></el-input>
        </el-form-item>
        <el-form-item label="账户密码" prop="loginAccount"  >
          <el-input v-model="pwd.passwordConfirm" type="password" style="min-width: 200px;max-width: 800px"></el-input>
        </el-form-item>
      </el-form>
      <div style="text-align:right;">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      catProduct(product){
        window.location.href="#/product-management/cat-product-release?productID="+product.productID
      },
      confirmUser(){
      ...
      handleModifyStatus(row) {
        this.$confirm('此操作删除账户【'+row.userName+'】, 是否继续?', '提示', {
