    ...
    <el-form :model="form" label-width="80px" ref="form">
      <el-row>
        <el-form-item :required="true" label="产品分类">
          ...
          <el-select :disabled="pageType=='cat'" placeholder="请选择三级分类" v-model="levelThreeCategoryID">
            <el-option
              :key="item.categoryID"
              :label="item.categoryName"
              :value="item.categoryID"
      ...
      <el-row>
        <el-col :span="20">
          <el-form-item :required="true" label="产品简介">
            <el-input :disabled="pageType=='cat'" type="textarea" v-model="form.product.productDescription"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="20">
          <el-form-item :required="true" label="主图视频">
            <el-col :span="4">
              <update-video :disabled="pageType=='cat'" :id="1" :img-type="10" :img-url="form.product.titleVideo"
                            @add-image="addVideo($event)" @remove-image="removeVideo($event)"/>
            </el-col>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24">
          <el-form-item :required="true" label="产品图片">
            <el-row>
              <el-col :span="4">
                <update-img :disabled="pageType=='cat'" :id="1" :img-type="1" :img-url="form.product.titleImg"
                            @add-image="addImage($event)" @remove-image="removeImage($event)"/>
              </el-col>
              <el-col :span="4">
                <update-img :disabled="pageType=='cat'" :id="2"
                            :img-type="2"
                            :img-url="form!=null&&form.commonImage!=null&&form.commonImage.length>=1?form.commonImage[0].imageUrl:''" @add-image="addImage($event)"
                            @remove-image="removeImage($event)"/>
              </el-col>
              <el-col :span="4">
                <update-img :disabled="pageType=='cat'" :id="3"
                            :img-type="2"
                            :img-url="form!=null&&form.commonImage!=null&&form.commonImage.length>=2?form.commonImage[1].imageUrl:''" @add-image="addImage($event)"
                            @remove-image="removeImage($event)"/>
              </el-col>
              <el-col :span="4">
                <update-img :disabled="pageType=='cat'" :id="4"
                            :img-type="2"
                            :img-url="form!=null&&form.commonImage!=null&&form.commonImage.length>=3?form.commonImage[2].imageUrl:''" @add-image="addImage($event)"
                            @remove-image="removeImage($event)"/>
              </el-col>
              <el-col :span="4">
                <update-img :disabled="pageType=='cat'" :id="5"
                            :img-type="2"
                            :img-url="form!=null&&form.commonImage!=null&&form.commonImage.length>=4?form.commonImage[3].imageUrl:''" @add-image="addImage($event)"
                            @remove-image="removeImage($event)"/>
              </el-col>
            </el-row>
      ...
      <el-row>
        <el-col :span="20">
          <el-form-item :required="true" label="产品详情">
            <tinymce :height="300" v-if="pageType!='cat'" v-model="form.productDetail.appDescription"/>
            <div style="max-height: 300px;overflow-y:auto;" v-else v-html="form.productDetail.appDescription"></div>
          </el-form-item>
        </el-col>
      </el-row>
        ...
        <el-col :span="20">
          <el-form-item :required="true" label="产品规格">
            ...
            <el-row>
              ...
              <el-row :gutter="10" style="margin-bottom: 10px" v-for="(item,index) in standardList"
                      v-if="standardList!=null&&standardList.length>0">
                <el-col :span="4">
                  <el-input :disabled="pageType=='cat'" placeholder="例 1.5*2.0" v-model="item.standard"></el-input>
                </el-col>
                <el-col :span="4">
                  <el-input :disabled="pageType=='cat'" placeholder="例 1200" v-model="item.bandPrice">
                    <template slot="append">元</template>
                  </el-input>
                </el-col>
                <el-col :span="4">
                  <el-input :disabled="pageType=='cat'" placeholder="例 1200" v-model="item.tradePrice ">
                    <template slot="append">元</template>
                  </el-input>
                </el-col>
                <el-col :span="11">
                  <el-row :gutter="10" v-for="(productStandardsColour,productStandardsColourIndex) in item.productStandardsColours"
                          v-if="item.productStandardsColours!=null&&item.productStandardsColours.length>0">
                    <el-col :span="3">
                      <update-img-for-standards :disabled="pageType=='cat'" :id="productStandardsColour.tempId"
                                                :img-type="5"
                                                :img-url="productStandardsColour.colourImage"
                                                @add-image="addStandardsColourImage($event)"/>
                    </el-col>
                    <el-col :span="9">
                      <el-input :disabled="pageType=='cat'" placeholder="例 象牙白"
                                v-model="productStandardsColour.colourName">

                      </el-input>
                    </el-col>
                    <el-col :span="8">
                      <el-input :disabled="pageType=='cat'" placeholder="库存"
                                v-model="productStandardsColour.productStockNum">
                        <template slot="append">件</template>
                      </el-input>
                    </el-col>
                    <el-col :span="4">
                      <el-button @click="removeProductStandardsColour(item,productStandardsColourIndex)" circle
                                 icon="el-icon-delete"
                                 type="danger" v-if="pageType!='cat'"></el-button>
                    </el-col>

                  </el-row>
                  <el-row>
                    <el-button @click="addProductStandardsColour(item)" circle icon="el-icon-plus"
                               type="primary" v-if="pageType!='cat'"></el-button>
                  </el-row>
        ...
        <el-col :span="20">
          <el-form-item :required="true" label="产品类型">
            <el-radio-group :disabled="pageType=='cat'" v-model="form.product.isSpecialGoods ">
              ...
              <el-select :disabled="pageType=='cat'" v-if="form.product.isSpecialGoods === 1"
                         v-model="form.product.vipProductType" value="1">
                <el-option
                  :key="1"
                  :value="1"
                  label="专享好物">
  ...
  export default {
    ...
    methods: {
      ...
      getProductDetail() {
        ...
        productDetail(queryParam).then(response => {
          if (response.status == 1) {
            ...
            if (response.data.productStandards != null && response.data.productStandards.length > 0) {
      ...
      onSubmit() {
        ...
        if (form.product.productDescription == null || form.product.productDescription == '') {
          this.$message({
            type: 'danger',
            message: '请填写产品简介！！'
          })
        ...
        if (form.productDetail.appDescription == null || form.productDetail.appDescription == '') {
          this.$message({
            type: 'danger',
            message: '请填写产品详情！！'
          })
        ...
        this.standardList.forEach(item => {
          ...
          item.productStandardsColours.forEach(productStandardsColour => {
            if (productStandardsColour.colourImage == null || productStandardsColour.colourImage == '') {
              this.$message({
                type: 'danger',
                message: '请上传产品规格颜色图片！！'
              })
            ...
            if (productStandardsColour.colourName == null || productStandardsColour.colourName == '') {
              this.$message({
                type: 'danger',
                message: '请填写产品规格颜色！！'
              })
            ...
            if (productStandardsColour.productStockNum == null || productStandardsColour.productStockNum == '') {
              this.$message({
                type: 'danger',
                message: '请填写产品规格颜色库存！！'
              })
