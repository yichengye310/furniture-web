<template>
  <div class="app-container">
    <div class="filter-container" v-if="searchStatus">
      ...
      <el-select v-model="listQuery.orderFrom" placeholder="订单来源" clearable class="filter-item" style="width: 130px">
        <el-option v-for="item in orderFromTypeOptions" :key="item.key" :label="item.display_name+'('+item.key+')'" :value="item.key" />
      </el-select>
    ...
    >
      ...
      <el-table-column label="下单时间" min-width="180px" align="center">
        <template slot-scope="{row}">
          <span v-if="row.orderPaidTime">{{ row.orderPaidTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
          <span v-else />
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态" class-name="status-col" min-width="200">
        <template slot-scope="{row}">
          <el-tag :type="row.orderState | orderStateStatusFilter">
            {{ row.orderState | orderStateFilter }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" min-width="100" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button size="mini" v-if="deltailStatus" @click="catOrderDetail(row)">
            详情
          </el-button>
          <el-button v-if="pageType==1&&row.orderState==1&&permission.indexOf('OrderDelivery765527')!=-1" size="mini" type="success" @click="orderShip(row)">
            发货
          </el-button>
          <el-button v-if="pageType==3&&permission.indexOf('OrderDelete50343')!=-1" size="mini" type="danger" @click="deleteOrder(row)">
            删除
          </el-button>
          <el-button v-if="pageType==4&&permission.indexOf('OrderProcess50345')!=-1" size="mini" type="primary" @click="finishedOrder(row)">
            已处理
          </el-button>
        </template>
      </el-table-column>
...
<script>
import { getOrderByState, deleteOrderByID, setOrderFinishedByID } from '@/api/order'
...
export default {
  ...
  created() {
    ...
    if((this.pageType===4&&this.permission.indexOf('OrderSearch43274')!=-1)//售后订单
      ||(this.pageType===3&&this.permission.indexOf('OrderSearch4274')!=-1)//订单已完成
      ||(this.pageType===99&&this.permission.indexOf('OrderSearch4234')!=-1)//订单搜索
      ||(this.pageType===2&&this.permission.indexOf('OrderSearch3423')!=-1)//待收货
    ...
    if((this.pageType===4&&this.permission.indexOf('OrderView53615')!=-1)//售后订单
      ||(this.pageType===3&&this.permission.indexOf('OrderView5365')!=-1)//订单已完成
      ||(this.pageType===99&&this.permission.indexOf('OrderView5345')!=-1)//订单搜索
      ||(this.pageType===2&&this.permission.indexOf('OrderView76657')!=-1)//待收货
  ...
  methods: {
    catOrderDetail(order) {
      window.location.href = '#/order-management/cat-order-detail?orderID=' + order.orderID
    },
    getList() {
    ...
    orderShip(row) {
      window.location.href = '#/order-management/order-ship?orderID=' + row.orderID
    },
    deleteOrder(row) {
      const that = this
      this.$confirm('此操作删除订单【' + row.orderID + '】, 是否继续?', '提示', {
    ...
    finishedOrder(row) {
      var that = this
      this.$confirm('此操作将订单【' + row.orderID + '】标记为已处理, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
