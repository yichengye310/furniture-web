      ...
      <el-row>
        <el-col :span="24" >
          <el-form-item label="产品图片" :required="true">
            <el-row>
              <el-col :span="4" >
                <update-img :img-type="8" :id="1"  :img-url="form!=null?form.activityImg:''" @add-image="addImage($event)" @remove-image="removeImage($event)"/>
              </el-col>
              <el-col :span="4"  >
                <update-img :img-type="9" :id="2" :img-url="form!=null&&form.commonImage!=null&&form.commonImage.length>=1?form.commonImage[0].imageUrl:''"  @add-image="addImage($event)" @remove-image="removeImage($event)"/>
              </el-col>
              <el-col :span="4"  >
                <update-img :img-type="9" :id="3" :img-url="form!=null&&form.commonImage!=null&&form.commonImage.length>=2?form.commonImage[1].imageUrl:''"  @add-image="addImage($event)" @remove-image="removeImage($event)"/>
              </el-col>
              <el-col :span="4" >
                <update-img :img-type="9" :id="4" :img-url="form!=null&&form.commonImage!=null&&form.commonImage.length>=3?form.commonImage[2].imageUrl:''"  @add-image="addImage($event)" @remove-image="removeImage($event)"/>
              </el-col>
              <el-col :span="4" >
                <update-img :img-type="9" :id="5" :img-url="form!=null&&form.commonImage!=null&&form.commonImage.length>=4?form.commonImage[3].imageUrl:''"  @add-image="addImage($event)" @remove-image="removeImage($event)"/>
              </el-col>
            </el-row>
        ...
        <el-col :span="discountWay==2?22:20">
          <el-form-item label="产品规格" :required="true">
            ...
            <el-row >
              ...
              <el-row :gutter="10" v-if="standardList!=null&&standardList.length>0" v-for="(item,index) in standardList" style="margin-bottom: 10px">
                <el-col :span="discountWay==2?3:4" >
                ...
                <el-col :span="11">
                  <el-row :gutter="10" v-if="item.productLimitedTimeStandardsColour!=null&&item.productLimitedTimeStandardsColour.length>0" v-for="(productStandardsColour,productStandardsColourIndex) in item.productLimitedTimeStandardsColour">
                    <el-col :span="3" >
                      <update-img-for-standards :img-type="5"  :id="productStandardsColour.tempId"  :img-url="productStandardsColour.colourImage" @add-image="addStandardsColourImage($event)" />
                    </el-col>
                    <el-col :span="13" >
                      <el-input placeholder="例 象牙白"  v-model="productStandardsColour.colourName">

                      </el-input>
                    </el-col>
                    <el-col :span="8" >
                      <el-button type="danger"  @click="removeProductStandardsColour(item,productStandardsColourIndex)" icon="el-icon-delete" circle></el-button>
                    </el-col>

                  </el-row>
                  <el-row>
                    <el-button type="primary" @click="addProductStandardsColour(item)" icon="el-icon-plus" circle></el-button>
                  </el-row>
  ...
  export default {
    ...
    mounted() {
      if(this.productItem!=undefined){
        ...
        console.log("mounted:" + JSON.stringify(this.form.productLimitedTimeStandardsDTOS));
        this.form=formTemp;
    ...
    methods: {
      ...
      onSubmit() {
        ...
        if(form.productLimitedTimeDetail.appDescription==null||form.productLimitedTimeDetail.appDescription==""){
          this.$message({
            type: 'danger',
            message: "请填写产品详情！！"
          });
        ...
        this.standardList.forEach(item =>{
          ...
          item.productLimitedTimeStandardsColour.forEach(productStandardsColour =>{
      ...
      addStandardsColourImage(img){
        ...
        this.standardList.forEach((standard, index) => {
          standard.productLimitedTimeStandardsColour.forEach((productStandardsColour, index) => {
            console.log("productStandardsColour.tempId: " + productStandardsColour.tempId + ", img.tempId: " + img.tempId);
