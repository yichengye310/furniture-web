...
<script>
  import {productLimitedTimeInfo, productLimitedTimeUpdate} from '@/api/marketing'
  ...
  export default {
    ...
    methods: {
      get () {
        productLimitedTimeInfo({ productLimitedTimeID: this.$route.query.id }).then(r => {
          if (r.status === 1) {
            this.activity.startTime = r.data.startTime * 1000
            this.activity.endTime = r.data.endTime * 1000
            this.activity.discountWay = r.data.discountWay
            this.activity.activeObject = r.data.activeObject
            this.activity.full = r.data.full / 100
            this.activity.reduce = r.data.reduce / 100
            this.form.productLimitedTimeDTOS = []
            this.form.productLimitedTimeDTOS.push(r.data)
            this.form.activityImg = r.data.activityImg
          }
        })
      },
      emptyProductList() {
      ...
      onSubmit() {
        ...
        if(flag){
          productLimitedTimeUpdate(this.form).then(response => {
