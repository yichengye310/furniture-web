<template>
  <div>
    <el-input @keyup.enter.native="handleFilter"
              class="filter-item"
              placeholder="优惠券名称" style="width: 200px;"
    ...
    >
      ...
      <el-table-column align="center" label="状态" min-width="120px">
        <template slot="header" slot-scope="scope">
          <el-cascader
            :options="stateList"
            :props="{ expandTrigger: 'hover' }" @change="getList"
            class="filter-item"
            clearable
            v-if="permission.indexOf('SearchCoupon1234')!=-1"
        ...
        <template slot-scope="{row}">
          <el-tag plain size="mini" type="info" v-if="row.couponState === 0">{{ row.couponState | statusFilter }}
          </el-tag>
          <el-tag plain size="mini" type="success" v-if="row.couponState === 1">{{ row.couponState | statusFilter }}
          </el-tag>
          <el-tag plain size="mini" type="danger" v-if="row.couponState === 9">{{ row.couponState | statusFilter }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" class-name="small-padding fixed-width" label="操作" min-width="200">
        <template slot-scope="{row}">
          <el-button @click="handleUpdate(row)" size="mini" type="primary"
                     v-if="permission.indexOf('editCoupon1234')!=-1">
            编辑
          </el-button>
          <el-button :disabled="row.couponState === 9" @click="handleUpdateTerminate(row)" plain size="mini" type="info"
    ...
    <pagination :limit.sync="listQuery.pageSize" :page.sync="listQuery.pageNo" :total="total"
                @pagination="getList"
                v-show="total>0"/>
  </div>
</template>
    ...
    methods: {
      getList() {
        ...
        if (this.listQuery.couponBeginTime) {
          this.listQuery.couponBeginTime = new Date(this.listQuery.couponBeginTime).getTime()
        }
        if (this.listQuery.couponEndTime) {
          this.listQuery.couponEndTime = new Date(this.listQuery.couponEndTime).getTime()
        }
      ...
      handleUpdate(row) {
        window.location.href="#/marketing/update-coupon?couponID=" + row.couponID
      },
