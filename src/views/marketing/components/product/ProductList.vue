    ...
    <el-form ref="form" label-width="80px" style="min-width: 1400px">
      <el-form-item label="活动信息">
        ...
        <el-button
          ...
          type="primary"
          @click="add()"
          v-if="(flag==2&&permission.indexOf('AddPosterMarketing5421')!=-1)||(flag==4&&permission.indexOf('AddHotRecommendation5421')!=-1)"
        >添加</el-button>
      </el-form-item>
    </el-form>
    ...
    >
      <el-form ref="form" :model="productDetail" label-width="80px" v-if="!isSelectProductStatus">
        <el-form-item label="图片">
          <update-img-for-poster-markeying
            :img-type="4"
            :id="productDetail.tempId"
            :img-url="productDetail.imageUrl"
        ...
        <el-form-item label="产品信息" v-if="productList.length>0">
          <el-table :data="productList" border style="width: 100%">
            ...
            <el-table-column width="180px" align="center" label="规格">
              <template slot-scope="{row}">
                <span v-for="item in row.productStandards">
                  {{ item.standard }}
                  <br />
                </span>
              </template>
            ...
            <el-table-column width="120px" align="center" label="销量">
              <template slot-scope="{row}">
      ...
      <div v-else>
        <div class="filter-container">
          ...
          ></el-cascader>
          <el-input
            v-model="selectProductListQuery.productName"
            placeholder="产品标题"
            style="width: 200px;"
            class="filter-item"
            @keyup.enter.native="getSelectProductList"
          />
          <el-button
            v-waves
            class="filter-item"
            type="primary"
            icon="el-icon-search"
            @click="getSelectProductList"
          >查询</el-button>
        </div>
        <el-table :data="selectProductList" border style="width: 100%">
          <el-table-column label="产品图片" width="125px" align="center">
            <template slot-scope="{row}">
              <img :src="row.titleImg" style="width: 100px;height: 100px" />
            </template>
        ...
        <pagination
          v-show="selectProductTotal>0"
          :total="selectProductTotal"
          :page.sync="selectProductListQuery.pageNo"
          :limit.sync="selectProductListQuery.pageSize"
...
export default {
  ...
  methods: {
        ...
        if (response.data.list.length > 0) {
          ...
          for (let i = 0; i < response.data.list.length; i++) {
            ...
            if (response.data.list[i].productCategoryLevelThrees.length > 0) {
              for (
                let j = 0;
                j < response.data.list[i].productCategoryLevelThrees.length;
                j++
              ) {
                ...
                categoryLevelTwoChild.label =
                  response.data.list[i].productCategoryLevelThrees[
                    j
                  ].categoryName;
                categoryLevelTwoChild.value =
                  response.data.list[i].productCategoryLevelThrees[
                    j
                  ].categoryID;
    ...
    getSelectProductList() {
      ...
      getProductListByPage(this.selectProductListQuery).then((response) => {
        ...
        response.data.list.forEach((item) => {
          ...
          if (
            item.productStandards != null &&
            item.productStandards.length > 0
          ) {
            item.bandPrice = item.productStandards[0].bandPrice;
            item.productStandards.forEach((productStandard) => {
              productStandard.productStandardsColours.forEach(
    ...
    onSubmit() {
      ...
      if (
        productDetail.relationID == undefined ||
        productDetail.relationID == ""
      ) {
        this.$message({
          type: "danger",
          message: "请选择产品!!!",
